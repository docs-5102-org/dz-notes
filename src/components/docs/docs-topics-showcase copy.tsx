...
import { ArrowRight, BookOpen, ChevronDown, Layers3, Search, X } from 'lucide-react';
...
export function DocsTopicsShowcase({ sections }: Props) {
  ...
  const activeSection =
    sections.find((section) => section.id === activeTopicId) ?? sections[0];
  ...
  const visibleSections = useMemo(() => {
    if (topicQuery.trim()) return filteredSections;
    if (showAllTopics || sections.length <= 6) return sections;
  ...
  function handleTopicKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    ...
    const currentIndex = visibleSections.findIndex((section) => section.id === activeSection.id);

    if (event.key === 'ArrowDown') {
      event.preventDefault();
      const nextIndex = currentIndex >= 0 ? (currentIndex + 1) % visibleSections.length : 0;
      setActiveTopicId(visibleSections[nextIndex].id);
    }
    ...
    if (event.key === 'ArrowUp') {
      ...
      setActiveTopicId(visibleSections[prevIndex].id);
    }
    ...
    <div className="mx-auto w-full max-w-[1440px] px-4 py-8 md:px-6 md:py-10">
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[0.5fr_1.5fr] xl:items-start">
        <aside className="min-w-0">
          <div
            ref={navRef}
            className="flex h-full flex-col gap-3 rounded-[28px] border border-fd-border bg-fd-card p-5 shadow-[0_18px_40px_rgba(0,0,0,0.08)]"
          >
            <div className="rounded-[20px] border border-fd-border bg-fd-card/50 p-3.5">
            ...
            <div className="w-full rounded-[22px] border border-fd-border bg-fd-card/55 p-2">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span
                          className={[
                            ...
                            isActive ? 'text-fd-popover-foreground' : 'text-fd-muted-foreground',
                          ].join(' ')}
                          title={section.meta}
                        >
            ...
            {!isSearching && !showAllTopics && sections.length > 6 ? (
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl border border-dashed border-fd-border bg-fd-card/60 px-4 py-3 text-sm font-medium text-fd-muted-foreground transition hover:border-fd-muted-foreground hover:bg-fd-card hover:text-fd-foreground"
              >
                展开全部频道
        ...
        <div className="min-w-0">
          <div className="rounded-[32px] border border-fd-border bg-fd-card p-6 shadow-[0_24px_60px_rgba(0,0,0,0.08)] md:p-8">
            <div
              className={[
                'relative overflow-hidden rounded-[30px] border border-fd-border p-6 md:p-8',
            ...
            >
              {activeSection.backgroundImage ? (
                <>
                  ...
                  />
                  <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(247,244,238,0.88),rgba(247,244,238,0.94))] dark:bg-[linear-gradient(180deg,rgba(18,18,18,0.76),rgba(18,18,18,0.88))]" />
                </>
              ) : (
                <div
                  className="absolute -right-12 -top-12 h-40 w-40 rounded-full opacity-70 blur-2xl"
              ...
              <div className="relative flex h-full flex-col">
                <div className="max-w-2xl">
                  ...
                  <h2 className="mt-3 text-[2rem] font-bold leading-tight tracking-[-0.04em] text-fd-foreground md:text-[2.4rem]">
                    {activeSection.title} 专题索引
                  </h2>
                  <p className="mt-4 text-[15px] leading-8 text-fd-popover-foreground md:text-base">
                ...
                <div className="mt-auto border-t border-fd-border/80 pt-6">
                  <div className="flex items-center justify-between gap-4">
                    <div className="rounded-2xl border border-fd-border bg-fd-card px-4 py-4">
                      <span className="block text-xs uppercase tracking-[0.14em] text-fd-muted-foreground">
                        收录文档
                      </span>
                      <span className="mt-2 block text-xl font-semibold text-fd-foreground">
                        {activeSection.articleCount}
                      </span>
                    </div>
                    <Link
                      href={activeSection.href}
                      className="inline-flex min-w-[180px] items-center justify-center gap-2 rounded-2xl border border-fd-border bg-fd-card px-5 py-4 text-sm font-semibold text-fd-foreground transition-colors hover:border-fd-muted-foreground hover:bg-fd-muted"
                    >
                      进入频道
            ...
            <div className="my-5 h-px bg-[linear-gradient(90deg,transparent,var(--color-fd-border),transparent)]" />
